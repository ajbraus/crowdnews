...
angular.module('CrowdNews.interceptors', []).
     ...
     return {
       ...
       'request': function(config) {
        ...
        if ((new RegExp(HOST)).test(config.url)) {
          ...
          //config.url += "&" + "auth_token=" + localStorage.getItem('crowdnews_auth_token');
          if (config.params != undefined) {
            config.params.auth_token = localStorage.getItem('crowdnews_auth_token');  
          } else {
            config.params = { 'auth_token' : localStorage.getItem('crowdnews_auth_token')};
          }
      ...
      'responseError': function(rejection) {
        ...
        // navigator.notification.activityStop();
         ...
         if (rejection.status == 401) {
           localStorage.removeItem('crowdnews_auth_token');
